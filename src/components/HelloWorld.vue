...
import cursor from '../assets/images/cursor-RmLDFyYc.png';

import glbModel from "./Sports Stadium.glb";
...
export default {
    ...
    methods: {
        checkCesiumLoaded() {
            ...
            const intervalId = setInterval(() => {
                if (typeof Cesium !== 'undefined') {
                    ...
                    var tileLoadProgressEvent = vm.viewer.scene.globe.tileLoadProgressEvent;
                    eventHelper.add(tileLoadProgressEvent, function (e) {
                        // console.log('每次加载地图服务矢量切片都会进入这个回调', e);
                        if (e == 0) {
                            console.log("矢量切片加载完成时的回调");
                            vm.loadingShow = false;//关闭加载动画
        ...
        initializeCesium() {
            ...
            // 设置最小缩放距离（以米为单位）
            vm.viewer.scene.screenSpaceCameraController.minimumZoomDistance = 500; // 例如设置为 1000 米

            // 设置最大缩放距离（以米为单位）
            vm.viewer.scene.screenSpaceCameraController.maximumZoomDistance = 8000000; // 例如设置为 5000000 米
            ...
            // let model = vm.viewer.entities.add({
            //     id: '建筑模型',
            ...
            // 监听地图鼠标移动事件
            // vm.viewer.screenSpaceEventHandler.setInputAction(vm.onMouseMove, Cesium.ScreenSpaceEventType.MOUSE_MOVE);



            // // 监听地图上的鼠标滚动事件
            // vm.viewer.scene.canvas.addEventListener('wheel', onMouseWheel);
            ...
            // // 监听鼠标滚动结束事件
            // vm.viewer.scene.canvas.addEventListener('wheel', onMouseWheelEnd);
        ...
        onEntityClick(movement) {
            ...
            if (pickedObject && pickedObject.id && pickedObject.id.monitoItems) {
                ...
                //====取消旋转======
                vm.viewer.clock.onTick.removeEventListener(vm.rotate);
        ...
        startRotation() {
            ...
            vm.viewer.zoomTo(position, offset).then(function () {
                vm.viewer.clock.onTick.addEventListener(vm.rotate);
            });
...
.mapView {
    ...
    height: 100%;
    overflow: hidden;
    cursor: url('../assets/images/cursor-RmLDFyYc32.png') 24 24, auto;
}
...
.clickPopup-title {
    ...
    height: 35px;
    line-height: 32px;
    background: url('../assets/images/custom-styles-title-BXpsbnaH.png');
...
.headView {
    width: 100%;
    height: 80px;
    background: url('../assets/images/header-CE2FZDIm.png');
    background-size: 100% 100%;
    position: fixed;
    top: -2px;
    z-index: 9;
...
.MouseZoom {
    ...
    background: url("../assets/images/cursor-RmLDFyYc.png");
    background-size: 100% 100%;
    z-index: 1;
...
.titleIcon {
    ...
    background: url("../assets/images/panel-header2-D01NbEeL.png");
    background-size: 100% 100%;
}
...
.boxTitle {
    ...
    color: #ffffff;
    background: url("../assets/images/panel-header1-DVTX7Lb2.png");
    background-size: 100% 100%;
...
.boxMenuView {
    ...
    ul {
        ...
        li {
            ...
            font-size: 18px;
            color: #ffffff;
            background: url('../assets/images/menu-btn-unchecked-BbOiNvmo.png');
            background-size: 100% 100%;
        ...
        li:nth-child(1) {
            background: url('../assets/images/menu-btn-checked_2-DcHCg5DW.png');
            background-size: 100% 100%;
            color: #ffdea3;
